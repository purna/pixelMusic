// Node.js compatible test script to demonstrate the fix for chance node properties
const fs = require('fs');

console.log("Testing chance node properties fix...");

// Load the property schema using fs (Node.js compatible)
try {
    const propertySchema = JSON.parse(fs.readFileSync('strudel-node-properties.json', 'utf8'));
    ...
    console.log(JSON.stringify(propertySchema.transformNodes.sometimes, null, 2));
